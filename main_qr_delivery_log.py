...
api_key = "5b3ce3597851110001cf6248019baed3eeca5edb8101"
...
class QRScanner(BoxLayout):
    ...
    def start_scan(self, instance):
        threading.Thread(target=self.scan_qr).start()
    ...
    def view_log_popup(self, instance):
        ...
        for idx, entry in enumerate(self.log):
            summary = f"{entry.get('Box_Dimension', '?')} | {entry.get('Weight', '?')} | {entry.get('Fragility', '?')} | {entry.get('Address', '')}"
    ...
    def send_logs_to_pc(self, instance):
        ...
        fields = ['Fragility', 'Box_Dimension', 'Stack_Lock', 'Weight', 'Address', 'Latitude', 'Longitude']
            ...
            try:
                ...
                for entry in ordered:
                    line = (
                        ...
                        f"Stack: {entry.get('Stack_Lock', '?')}, Addr: {entry.get('Address', '')}"
                    )
